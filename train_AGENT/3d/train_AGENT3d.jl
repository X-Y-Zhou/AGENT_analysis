...
# Parameter truth order: [sigma_on, sigma_off, rho, dm, lambda, dp].
# n_z = 7; SSA targets were generated with n_t = 10^4 trajectories.
const NODES_PER_AXIS = 7
const ADAM_STEPS = 3350
const LBFGS_STEPS = 21000
const HIDDEN_WIDTH = 160
const INPUT_SIZE = 10
const OUTPUT_SIZE = 343
const TRAINING_GROUPS = 2751
...
function training_data()
    ...
    target = readdlm(joinpath(@__DIR__, "matrix_Gz1z2z3.csv"), Float64)
    @assert size(truth) == (TRAINING_GROUPS, 6)
    ...
    w = weights ./ 2
    W = vcat([vec(w * w') * w[i] for i in 1:NODES_PER_AXIS]...)
...
function initialize(input; seed=20260913)
    Random.seed!(seed)
...
learning_rate(t) = 0.03 * 0.1^((t - 1) / 3349)

function train(; output=joinpath(@__DIR__, "outputs"), seed=20260913,
    ...
    CSV.write(joinpath(output, "params_trained3d.txt"), DataFrame(params=folded_parameters))
