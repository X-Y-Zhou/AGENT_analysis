...
w = weights ./ 2
W = vec(w * transpose(w))
core = Chain(Dense(8, 80, tanh), Dense(80, 49))
core = Flux.f64(core)
...
agent = AGENT.AGENTModel(2, z1, z2, z3, W, network,
    Float64.(trained_params), 1.0, 1)
...
counts_path = joinpath(@__DIR__, "..", "dataset", "synthetic_data", "counts_example2d.txt")
...
opt = Optim.optimize(q -> objective(q, SSA_PGF), zeros(4), Optim.Options(show_trace=false, g_tol=1e-11, iterations=1000))
